...
const prevSpeakers = [
  ...
  { name: "Mohammed Ungsi", org: "Embassy Indonesia · Abu Dhabi", init: "MU" },
  { name: "Kristin Surak", org: "Prof. London School of Econ.", init: "KS" },
...
export default function PartnersAndAgenda() {
  return (
    <>
      {/* PARTNERS */}
      <section id="partners-prev" className="py-16 border-b border-gray-100" style={{ background: "#f9fafb" }}>
        <div className="container">
          ...
          <AnimatedSection delay={100}>
            <div className="grid grid-cols-3 md:grid-cols-5 gap-2 mb-10">
              {prevPartners.map((p, i) => (
                <div key={i} className="bg-white border border-gray-200 rounded-lg py-3 px-3 text-center text-xs font-semibold text-gray-600 hover:border-gray-300 transition-colors">{p}</div>
              ))}
            </div>
          </AnimatedSection>

          <AnimatedSection delay={200}>
            <p className="font-bold text-gray-900 mb-4">Our Previous Speakers</p>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-10">
              {prevSpeakers.map((s, i) => (
                <div key={i} className="text-center">
                  <div className="w-12 h-12 rounded-full flex items-center justify-center font-black text-sm mx-auto mb-2 text-white" style={{ background: "#16a34a" }}>{s.init}</div>
          ...
          <AnimatedSection delay={300}>
            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Media Coverage</p>
            <div className="flex flex-wrap gap-2">
              {mediaOutlets.map((m, i) => (
                <span key={i} className="px-2.5 py-1 bg-white border border-gray-200 rounded-md text-xs font-medium text-gray-500">{m}</span>
              ))}
            </div>
          </AnimatedSection>
        </div>
      ...
      {/* AGENDA */}
      <section id="agenda" className="py-16 border-b border-gray-100">
        <div className="container">
          <AnimatedSection>
            <p className="section-label">Summit Agenda</p>
              ...
              <div key={di}>
                ...
                <div className="space-y-2">
                  {day.sessions.map((s, i) => (
                    <AnimatedSection key={i} delay={100 + di * 100 + i * 60}>
                      <div className="card hover:shadow-sm transition-shadow">
                        <div className="flex items-start gap-3">
                          <span className="font-black text-gray-200 text-lg w-5 flex-shrink-0">{s.num}</span>
