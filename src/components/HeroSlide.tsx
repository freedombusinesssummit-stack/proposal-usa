...
function Countdown() {
  ...
  return (
    <div className="flex items-center gap-1 font-black tracking-tighter text-carbon-900" style={{ fontSize: "clamp(44px, 8vw, 72px)", letterSpacing: "-3px" }}>
      <span>{pad(t.d)}</span><span style={{ color: "#9ef01a", opacity: 0.7 }}>:</span>
      <span>{pad(t.h)}</span><span style={{ color: "#9ef01a", opacity: 0.7 }}>:</span>
      <span>{pad(t.m)}</span><span style={{ color: "#9ef01a", opacity: 0.7 }}>:</span>
      <span>{pad(t.s)}</span>
    </div>
  );
}
...
export default function HeroSlide() {
  ...
  return (
    <section id="hero" className="pt-14 bg-white">
      <div className="wrap">
        <div className="py-12 md:py-16">
          ...
          <div style={f(80)}>
            ...
            <h1 className="font-black text-carbon-900 leading-none mb-3" style={{ fontSize: "clamp(44px, 7vw, 84px)", letterSpacing: "-2px" }}>
              USA Mobility Edition 🇺🇸
            </h1>
            <h2 className="font-bold text-carbon-800 leading-snug mb-6" style={{ fontSize: "clamp(18px, 2.5vw, 26px)" }}>
              A Data-Driven Virtual Summit for Founders,<br />Investors Exploring Inbound & Outbound Strategies
            </h2>
          </div>
          ...
          <div className="mb-8" style={f(250)}><Countdown /></div>
          ...
          <div className="flex items-center gap-3" style={f(380)}>
            <div className="flex -space-x-2">
              {["DD","PM","SZ","DC","WW","MC","AR","MU"].map((init, i) => (
                <div key={i} className="w-9 h-9 rounded-full border-2 border-white flex items-center justify-center text-white text-xs font-black" style={{ background: `hsl(${130 + i * 15}, 55%, 35%)`, zIndex: 8 - i }}>{init}</div>
              ))}
            </div>
      ...
      {/* Meta stats — separate blocks */}
      <div className="border-t border-carbon-100">
        <div className="wrap">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 py-5">
            {META.map((m, i) => (
              <div key={i} className="flex items-center gap-3 p-4 rounded-xl border border-carbon-200 bg-white hover:border-lime-300 transition-colors">
                <span className="text-2xl flex-shrink-0">{m.icon}</span>
                <span className="font-semibold text-sm text-carbon-700">{m.label}</span>
              </div>
            ))}
          </div>
        </div>
