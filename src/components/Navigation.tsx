      ...
      <div className="wrap">
        <div className="flex items-center justify-between h-14">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-md flex items-center justify-center font-black text-xs" style={{ background: "#9ef01a", color: "#111827" }}>FS</div>
          ...
          <div className="hidden lg:flex items-center gap-5">
            {[
              ...
              <button key={l.id} onClick={() => go(l.id)}
                className={`text-sm font-medium transition-colors ${active === l.id ? "text-lime-400" : "text-carbon-500 hover:text-carbon-800"}`}>
                {l.label}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <a href="mailto:denis@fsummit.net" className="btn-green hidden sm:inline-flex text-sm py-2 px-4">Partner With Us →</a>
        ...
        <div className="lg:hidden border-t border-carbon-100 bg-white py-2">
          {SLIDES.map(s => (
            <button key={s.id} onClick={() => go(s.id)}
              className={`w-full text-left px-6 py-2.5 text-sm font-medium transition-colors ${active === s.id ? "text-lime-400 bg-lime-50" : "text-carbon-600 hover:bg-carbon-50"}`}>
              {s.label}
            </button>
          ))}
        </div>
